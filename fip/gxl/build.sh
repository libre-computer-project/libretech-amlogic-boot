...
function build_fip() {
	...
	${FIP_FOLDER}/acs_tool.py ${BUILD_PATH}/${AML_BL2_NAME} ${BUILD_PATH}/bl2_acs.bin ${BUILD_PATH}/acs.bin 0
...
function package() {
	...
	if [ "y" == "${CONFIG_AML_SIGNED_UBOOT}" ]; then
		if [ -e "${BUILD_PATH}/bl2.v3.bin" ]; then
			# acs_tool process ddr timing and configurable parameters
			${FIP_FOLDER}/acs_tool.py ${BUILD_PATH}/bl2.v3.bin ${BUILD_PATH}/bl2_acs.bin ${BUILD_PATH}/acs.bin 0
			# fix bl2/bl21
			fix_blx \
				${BUILD_PATH}/bl2_acs.bin \
				${BUILD_PATH}/zero_tmp \
